...
var links = [];
var force2;
var link, node, anchorLink, anchorNode;
var arrows = []
...
d3.json("/getCrushGraphData",function (alldata) {
	...
	for(var j = 0; j<c.length; j++) {
		...
		var nn = nodes[x]['label'].toLowerCase();
		if (nn==name) {
		    li = x;
		    break;
		}
    ...
    force2 = d3.layout.force().nodes(labelAnchors).links(labelAnchorLinks).gravity(0).linkDistance(0).linkStrength(8).charge(-100).size([w, h]);
    force2.start();

    link = vis.selectAll("line.link").data(links).enter()
.append("svg:line").attr("class", "link").attr("marker-end","url(#arrow)").style("stroke", "#F00")
;
